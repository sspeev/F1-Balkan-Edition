...
namespace EmotivUnityPlugin
{
    ...
    public class DataStreamManager
    {
        ...
        public List<Headset>  DetectedHeadsets = new List<Headset>(); // list of detected headsets
        ...
        private bool isConnectedHeadset(string headsetId) 
        {
            lock (_locker)
            {
                foreach (var item in DetectedHeadsets) {
        ...
        public void StartDataStream(List<string> streamNameList, string headsetId)
        {
            lock (_locker)
            {
                ...
                if (string.IsNullOrEmpty(headsetId)) {
                    if (DetectedHeadsets.Count > 0) {
                        // get the first headset in the list
                        _wantedHeadsetId = DetectedHeadsets[0].HeadsetID;
                    }
                    else {
